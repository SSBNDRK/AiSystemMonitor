...
namespace AiSystemMonitor
{
    public partial class MainWindow
    {
        private readonly Dictionary<string, string> _slashCommands = new()
        {
            ...
            { "/аналіз", "Короткий аналіз ПК та поради" },
            { "/процеси", "Топ процесів за споживанням ОЗП" },
            { "/процес", "Інформація про конкретний процес" },
            { "/датчики", "Температури, навантаження та кулери" },
            { "/диски", "Вільне місце та стан дисків" },
            { "/тест диска", "Швидкий тест запису на диск" },
            { "/стартап", "Програми автозавантаження зі статусом" },
            { "/живлення", "Показати режими електроживлення Windows" },
            { "/буст", "Увімкнути максимальну продуктивність і закрити браузери" },
            { "/оптимізація", "Знайти важкі фонові програми для закриття" },
            { "/мережа", "Діагностика інтернету, DNS і шлюзу" },
            { "/пінг", "Перевірити затримку інтернету" },
            { "/очистка", "Безпечне очищення тимчасових файлів" },
            { "/краші", "Перевірити останні збої, BSOD і вильоти драйверів" }
        };
